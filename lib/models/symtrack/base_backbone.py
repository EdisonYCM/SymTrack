...
from lib.models.symtrack.utils import combine_tokens, recover_tokens
...
class BaseBackbone(nn.Module):
    ...
    def forward_features(self, z, x, track_query=None, token_type="add"):
        ...
        # Create an empty list to store all attention maps
        all_attns = [] 

        for i, blk in enumerate(self.blocks):
            x, attn = blk(x, lens_z, lens_x, return_attention=True)
            all_attns.append(attn) # Save the attention map from each layer during the loop
        ...
        aux_dict = {
            # "attn_first": all_attns[0],                   # Layer 0
            # "attn_mid": all_attns[num_blocks // 2 - 1],   # Middle layer
            "attn_last": all_attns[-1]                    # Last layer
        }
