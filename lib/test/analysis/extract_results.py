...
def extract_results(trackers, dataset, report_name, skip_missing_seq=True, plot_bin_gap=0.05,
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            ...
            current_seq_score = avg_overlap_all[seq_id, trk_id].item()

            sequence_scores.append({'name': seq.name, 'score': current_seq_score})
    ...
    print("\n--- Per-Sequence Average Overlap Scores ---")
    
    output_txt_path = os.path.join(settings.result_plot_path, report_name, f"{report_name}_sequence_scores.txt")

    with open(output_txt_path, 'w', encoding='utf-8') as f:
        f.write("--- Per-Sequence Average Overlap Scores ---\n")

        sorted_scores = sorted(sequence_scores, key=lambda x: x['score'])
        for item in sorted_scores:
            line = f"Sequence: {item['name']}, Average IoU: {item['score']:.4f}\n"
            print(line.strip())  
            f.write(line)       

    print(f"\n✅ Sequence scores have been saved to: {output_txt_path}\n")
