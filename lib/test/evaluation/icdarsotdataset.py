...
class ICDARSOTDataset(BaseDataset):
    """
    ICDAR_SOT_Train dataset, created for single object tracking evaluation.
    Follows the design pattern of official GOT-10k test dataset loader.
    """
    def __init__(self):
        super().__init__()
        self.base_path = self.env_settings.icdar_sot_dir
    ...
    def _construct_sequence(self, sequence_name):
        ...
        # Load ground-truth bounding boxes (AABB format: [x, y, w, h])
        ground_truth_rect = load_text(str(anno_path), delimiter=',', dtype=np.float64)

        # Get and sort the filenames of all frame images
        frame_list = [frame for frame in os.listdir(frames_path) if frame.endswith(".jpg")]
        # Sort by filename after removing the extension, e.g. 00000001.jpg -> 1
        ...
        # Note that the third argument dataset name is changed to 'icdar_sot'.
        return Sequence(sequence_name, frames_list_full, 'icdar_sot', ground_truth_rect.reshape(-1, 4))
