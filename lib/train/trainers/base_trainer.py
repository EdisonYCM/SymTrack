...
class BaseTrainer:
    ...
    def train(self, max_epochs, load_latest=False, fail_safe=True, load_previous_ckpt=False, distill=False):
        ...
        """
        if self.actor.cfg.TRAIN.FINETUNE_FROM:
            print(f"INFO: Loading checkpoint for fine-tuning from: {self.actor.cfg.TRAIN.FINETUNE_FROM}")
            self.load_checkpoint(self.actor.cfg.TRAIN.FINETUNE_FROM, fine_tune=True)

            load_latest = False
            print("INFO: Fine-tune checkpoint loaded. Starting training from epoch 1.")
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False, fine_tune=False):
        ...
        if fine_tune:
            print("INFO: Fine-tuning mode. Ignoring 'optimizer' and 'epoch' from checkpoint.")
            ignore_fields.extend(['optimizer', 'epoch', 'stats'])
        ...
        for key in fields:
            ...
            if key == 'net':
                ...
                print("INFO: Loading state dict in non-strict mode.")
