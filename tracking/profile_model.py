...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='symtrack', choices=['symdtrack'],
                        help='training script name')
    parser.add_argument('--config', type=str, default='baseline_text_scalear', help='yaml configure file name')
    args = parser.parse_args()

    return args


def evaluate_symtrack(model, template, search):
    '''Speed Test'''
...
if __name__ == "__main__":
    ...
    if args.script == "symtrack":
        ...
        if merge_layer <= 0:
            evaluate_symtrack(model, template_list, search_list)
        else:
